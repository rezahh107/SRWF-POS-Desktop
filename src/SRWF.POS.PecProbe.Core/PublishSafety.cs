...
public static partial class RequestTemplateRenderer
{
    [GeneratedRegex(@"^(?<key>[^=\r\n]+)=(?<value>.*)$", RegexOptions.Multiline)]
    private static partial Regex KeyValueLineRegex();

    public static RequestRenderResult Render(ObservedRequestContract contract, long rawAmount, IReadOnlyDictionary<string, string>? supportedOverrides = null)
    {
        ...
        var rendered = KeyValueLineRegex().Replace(contract.TextEvidence.Text, match =>
        {
            var key = match.Groups["key"].Value;
            if (!replacements.TryGetValue(key, out var value)) return match.Value;
            replaced.Add(key);
            return key + "=" + value;
        });
        ...
        var bytes = EvidenceTextAnalyzer.EncodeLike(contract.TextEvidence, rendered);
...
{
    public static RequestComparison Compare(ReadOnlySpan<byte> observed, ReadOnlySpan<byte> prepared)
    {
        ...
        var newlineSame = o.NewlineRepresentation == p.NewlineRepresentation;
        ...
        var result = byteIdentical
            ? "BYTE_IDENTICAL"
            : encodingSame && bomSame && newlineSame && orderSame && finalNewlineSame
                ? "STRUCTURALLY_EQUIVALENT"
                : o.DecodingSafe && p.DecodingSafe ? "DIFFERENT" : "NOT_COMPARABLE";

        return new(result, encodingSame, bomSame, newlineSame, orderSame, finalNewlineSame, byteIdentical,
            Hashing.Sha256(observed), Hashing.Sha256(prepared),
            ["Dynamic transactions must not be labelled byte-identical unless the exact bytes match."]);
    }
}
